import cia_redaction_module as crm
import cv2
import numpy as np
import sys
...
def show_individual_page(jpg_file):
    ...
    [redaction_count, redacted_text_area, estimated_text_area, estimated_num_words_redacted, is_map] = crm.image_processing(jpg_file)
    ...
    (potential, text_potential) = crm.get_redaction_shapes_text_shapes(contours, thresh)
    final_redactions, is_map = crm.get_intersection_over_union(potential)
    crm.drawRedactionRectangles(final_redactions, img)
    crm.putRedactions(final_redactions, img)
